...
`Assets/Editor/RoomSenseSetup.cs` builds the whole test scene headlessly and
`RoomSensePlayTest.cs` runs it in play mode and asserts on the result — no clicking, no Quest:
