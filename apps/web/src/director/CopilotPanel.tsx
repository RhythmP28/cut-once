...
import { fetchAnswerPcm, fetchLastFrame, getLastCapture, type LastCapture } from "../api";
...
/** Plays the answer's raw PCM. WebAudio, because `<audio>` cannot play a headerless stream. */
function useAnswerAudio() {
  ...
  const play = useCallback(async (turnId: string) => {
    setState("loading");
    try {
      const pcm = await fetchAnswerPcm(turnId);
      if (!pcm || pcm.samples.length === 0) { setState("none"); return; }
      ctxRef.current ??= new AudioContext();
      const audio = ctxRef.current;
      await audio.resume();
      const buffer = audio.createBuffer(1, pcm.samples.length, pcm.rate);
