...
- [ ] **Step 5: Test your real server the same way, never during a demo.** The scenario starts a new run and marks the crossbar wrong, and every connected headset follows, so `--base` refuses to run without `--allow-live`. With `pnpm serve:local` running and no headset attached: `pnpm -F @cutonce/api exec tsx src/cli/sim-run.ts --base http://127.0.0.1:8080 --token <your API_TOKEN> --allow-live --out /tmp/real.json`. Expect everything to pass except the copilot steps, unless that server also runs with `COPILOT_MODE=fake`. Afterwards, start a fresh run from `/director`.
