...
import { annotateFrame, type LegendRow } from "./annotate.js";
...
export interface Deps { ctx: Ctx; models: CopilotModels; speech: Speech; turns: Turns; cache: DemoCache }
...
/**
 * Applies a spoken command. The server owns the event log, so it writes the event and the headset
...
export async function answerQuery(deps: Deps, input: QueryInput, log: Log): Promise<CopilotResponse> {
  ...
  if (input.context.scripted_query_id) {
    ...
    if (hit) {
      ...
      turns.record({ response, assembly_id: input.assemblyId, selected_part_id: input.context.selected_part_id, chunk_ids: [], scripted_query_id: input.context.scripted_query_id, asked_at: new Date().toISOString() });
      return response;
    }
  }
  ...
  if (fast) {
    ...
    turns.record({ response, assembly_id: input.assemblyId, selected_part_id: input.context.selected_part_id, chunk_ids: [], scripted_query_id: null, asked_at: new Date().toISOString() });
  ...
  const [chunks, annotated] = await Promise.all([
    ...
    Promise.resolve().then(() => {
      ...
      try {
        const out = annotateFrame(input.frame, input.context.visible_parts);
  ...
  const legend: LegendRow[] = annotated?.legend ?? input.context.visible_parts.map((v, i) => ({ n: i + 1, part_id: v.part_id, state: v.state, in_frame: v.in_frame, distance_m: v.distance_m }));
  ...
  const result = await withCap(
    ask(ctx, m, {
      ...
      frames: { annotated: annotated?.jpeg ?? null, raw: input.frame },
    }),
  ...
  if (!result) {
    ...
    return capped(deps, turnId, transcript, input, timings, t0, chunks);
  }

  const grounded = ground(result.draft, g, chunks);
  speech.start(turnId, grounded.answer_text);
  const response: CopilotResponse = {
    ...shell(turnId, transcript), ...grounded, action: null,
  ...
  turns.record({ response, assembly_id: input.assemblyId, selected_part_id: input.context.selected_part_id, chunk_ids: chunks.map((c) => c.chunk_id), scripted_query_id: null, asked_at: new Date().toISOString() });
...
function capped(deps: Deps, turnId: string, transcript: string, input: QueryInput, timings: Record<string, number>, t0: number, chunks: RetrievedChunk[]): CopilotResponse {
  ...
  deps.turns.record({ response, assembly_id: input.assemblyId, selected_part_id: input.context.selected_part_id, chunk_ids: chunks.map((c) => c.chunk_id), scripted_query_id: input.context.scripted_query_id, asked_at: new Date().toISOString() });
  return response;
}
