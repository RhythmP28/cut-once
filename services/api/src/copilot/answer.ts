...
import { toOpenAiSchema } from "../llm.js";
...
/**
 * What the model must return. Strict structured-output mode wants every field required, so nothing is
 * optional here; `cited_chunk_ids` carries the citation and the server turns it into drawing_refs, because
 * the model may not invent a document id that was never shown to it.
 */
const Draft = z.object({
  answer_text: z.string(),
  highlight_parts: z.array(z.string()),
  highlight_style: z.enum(["pulse", "path"]),
 ...
 * Section 10 budgets 1.5–3.0 s for this; the caller passes the 6 s timeout.
 */
export async function ask(ctx: Ctx, m: CopilotModels, input: AskInput): Promise<{ draft: Draft; toolCalls: string[] }> {
  ...
  const response_format = { type: "json_schema" as const, json_schema: { name: "copilot_answer", strict: true, schema: toOpenAiSchema(Draft) } };
  ...
  return { draft: Draft.parse(JSON.parse(text)), toolCalls };
}

const clamp01 = (n: number) => (Number.isFinite(n) ? Math.min(1, Math.max(0, n)) : 0.5);

/**
 * Section 10, t≈3.5: drop any part id that is not in the plan and any citation that was not in the
 * retrieved set. A hallucinated part id would make the headset highlight nothing and look broken;
 * a hallucinated source card would be a lie on screen in front of judges.
 */
export function ground(draft: Draft, g: Gathered, chunks: RetrievedChunk[]): Pick<CopilotResponse, "answer_text" | "highlight_parts" | "highlight_style" | "drawing_refs" | "confidence" | "needs_clarification"> {
  const known = new Set(g.plan.parts.map((p) => p.part_id));
  const byChunk = new Map(chunks.map((c) => [c.chunk_id, c]));
  const highlight_parts = [...new Set(draft.highlight_parts)].filter((id) => known.has(id));
  const drawing_refs = [...new Set(draft.cited_chunk_ids)].flatMap((id) => {
  ...
  return {
    ...
    drawing_refs,
    // An answer that named parts we do not have is worth less, and the Director page should show that.
